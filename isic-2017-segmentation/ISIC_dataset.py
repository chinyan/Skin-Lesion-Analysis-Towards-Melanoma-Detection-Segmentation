...
def get_labels(image_list, csv_file):
    ...
    # image_list = [filename for filename in image_list]
...
def get_color_image(image_name, image_folder, remove_mean_imagenet=True, use_hsv=False, remove_mean_samplewise=False):
    ...
    # img = cv2.imread(os.path.join(image_folder, image_name.replace(".jpg",".png")).replace("/","\\"))  # windows
    img = cv2.imread(os.path.join(image_folder, image_name.replace(".jpg",".png")))  # unix
